...
const staging = fs.mkdtempSync(path.join(os.tmpdir(), "cos-workbench-package-"));
...
function verifySignedMacArchive() {
  ...
  const verificationRoot = fs.mkdtempSync(path.join(os.tmpdir(), "cos-workbench-mac-verify-"));
  try {
