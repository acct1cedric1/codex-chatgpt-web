...
test("launcher publishes native packages for all supported desktop operating systems", () => {
  assert.equal(manifest.build.appId, "dev.cos.workbench");
  assert.equal(manifest.build.artifactName, "cos-workbench-${version}-${os}-${arch}.${ext}");
...
test("release installers resolve checksummed native launcher assets", () => {
  ...
  assert.match(shellInstaller, /cos-workbench\.desktop/);
  ...
  assert.match(windowsInstaller, /cos-workbench-\$Version-win-\$Arch\.exe/);
  ...
  assert.equal(fullyQualifiedWindowsPath.test("C:\\Users\\tester\\COS Workbench"), true);
  assert.equal(fullyQualifiedWindowsPath.test("\\\\server\\share\\COS Workbench"), true);
  assert.equal(fullyQualifiedWindowsPath.test("C:COS Workbench"), false);
  assert.equal(fullyQualifiedWindowsPath.test("\\COS Workbench"), false);
  assert.equal(fullyQualifiedWindowsPath.test("COS Workbench"), false);
...
test("CI packages and smoke-launches on macOS, Windows, and Linux", () => {
  ...
  assert.match(release, /COS Workbench\.app/);
...
test("Linux AppImage fallback uses one owned extraction and removes it on exit", {
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "cos-workbench-appimage-runner-"));
  const runtime = path.join(root, "runtime");
  const appImage = path.join(root, "COS Workbench.AppImage");
  ...
  const fallbackRoot = path.join(runtime, `cos-workbench-appimage-${process.getuid?.() ?? 0}`);
