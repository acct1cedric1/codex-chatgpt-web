...
REPOSITORY="${CODEX_WEB_GPT_REPOSITORY:-acct1cedric1/codex-chatgpt-web}"
...
VERSION="${VERSION#v}"
if [ -z "$VERSION" ]; then
  echo "Could not resolve the latest COS Workbench release" >&2
  exit 1
fi
case "$VERSION" in
...
ASSET="cos-workbench-$VERSION-$PLATFORM-$ARCH.$EXTENSION"
BASE_URL="https://github.com/$REPOSITORY/releases/download/v$VERSION"
TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/cos-workbench-launcher.XXXXXX")"
...
if [ "$OS" = "Darwin" ]; then
  ...
  SOURCE_APP="$STAGE_DIR/COS Workbench.app"
  if [ ! -d "$SOURCE_APP" ] || [ ! -x "$SOURCE_APP/Contents/MacOS/COS Workbench" ]; then
  ...
  TARGET_APP="$INSTALL_DIR/COS Workbench.app"
  if pgrep -x "COS Workbench" >/dev/null 2>&1; then
    echo "Quit COS Workbench before updating it" >&2
    exit 1
  fi
  BACKUP_APP="$TEMP_DIR/COS Workbench.previous.app"
...
LIB_DIR="${CODEX_WEB_GPT_LIB_DIR:-$HOME/.local/lib/cos-workbench}"
BIN_DIR="${CODEX_WEB_GPT_BIN_DIR:-$HOME/.local/bin}"
TARGET_DIR="$LIB_DIR/$VERSION"
TARGET="$TARGET_DIR/COS Workbench.AppImage"
WRAPPER="$BIN_DIR/cos-workbench"
CORE_HOME="${CODEX_CHATGPT_WEB_HOME:-$HOME/.cos-workbench}"
...
if { [ -n "$RUNNING_PID" ] && kill -0 "$RUNNING_PID" 2>/dev/null; } \
  || pgrep -f "COS Workbench\\.AppImage" >/dev/null 2>&1; then
  echo "Quit COS Workbench before updating it" >&2
  exit 1
fi
...
install -m 0644 "$ICON_SOURCE" "$ICON_DIR/cos-workbench.png"
DESKTOP_WRAPPER="$(printf '%s' "$WRAPPER" | sed \
  ...
  -e 's/\$/\\$/g' \
  -e 's/%/%%/g')"
cat > "$APPLICATIONS_DIR/cos-workbench.desktop" <<EOF
[Desktop Entry]
Type=Application
Version=1.0
Name=COS Workbench
...
StartupWMClass=cos-workbench
EOF
chmod 0644 "$APPLICATIONS_DIR/cos-workbench.desktop"
